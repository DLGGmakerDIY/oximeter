...
bool gpsParse(String input)
{
  ...
  if (input.startsWith("$GNZDA") && input.length() >= 38)
  {
    ...
    if (lastGpsTimeSet == 0 || TDIFFMIN(lastGpsTimeSet, 10))
    {
      ...
      DBG2("GPS Date & Time: " + String(gpsDateTime.tm_mon + 1) + "/" + String(gpsDateTime.tm_mday) + "/" + String(gpsDateTime.tm_year + 1900) + " " + String(gpsDateTime.tm_hour) + ":" + String(gpsDateTime.tm_min) + ":" + String(gpsDateTime.tm_sec) + " UTC");
...
void sendTimeSerial()
{
  ...
  DynamicJsonDocument SysPacket(256);
